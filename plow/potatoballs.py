...
def write_main_data(
    ...
    draw.text((WIDTH, SH + 201),  country,
              MFC, font=MF)  # Гражданство

    draw.text((WIDTH, SH + 268), birth_date,
    ...
    # else:
    #     g = 'Ж/F'
    draw.text((WIDTH, SH + 335), genre, MFC, font=MF)  # Пол

    draw.text((WIDTH + 489, SH + 268), birth_date +
              '-' + '1233', MFC, font=MF)  # Персональный номер

    draw.text((WIDTH + 160, SH + 335), str(country).upper(),
              MFC, font=MF)  # Город

    draw.text((WIDTH + 460, 770), str(personal_number).upper(), (178, 34, 34),
              font=ImageFont.truetype('media/cfg/font/sans.ttf', 40))

    draw.text((WIDTH, SH + 402), data_start,
              MFC, font=MF)  # Дата выдачи

    draw.text((WIDTH, SH + 469), exp_date,
...
def paste_photo(passport_temp_path, pasport_photo_path, exif_infor, background_path):
    ...
    all_done_path = f'media/cfg/out/usavisa{random.randint(1000,9999)}.png'
    print(all_done_path)
