...
def write_main_data(
    mrz, save_after_main_data, typedoc, country, passport_number, surname, given_name, genre, birth_date,
        data_start, exp_date, personal_number):
    """ Нанесение на шаблон документа основной информации """
    img = Image.open('media/cfg/template/uain.png')  # Шаблон паспорта
    ...
    draw.text(
        (WIDTH, SH + 67),
        str(surname + '/' + translit(surname, 'uk', reversed=True)).upper(),
        MFC, font=MF)  # Фамилия

    draw.text(
        (WIDTH, SH + 134),
        str(given_name + '/' + translit(given_name, 'uk', reversed=True)).upper(),
        MFC, font=MF)  # Имя влядельца документа

    draw.text((WIDTH, SH + 201),  'УКРАЇНА/UKRAINE',
              MFC, font=MF)  # Гражданство

    draw.text((WIDTH, SH + 268), str(dt(birth_date[2::])),
